...
if sys.platform.startswith('linux'):
    filename = os.path.join(LIB_PATH, 'linux', 'libsvm.so.2')
    libsvm = CDLL(filename)
elif sys.platform.startswith('darwin'):
    filename = os.path.join(LIB_PATH, 'mac', 'libsvm.so.2')
    libsvm = CDLL(filename)
else:
    libdir = 'win32' if platform.architecture()[0] == '32bit' else 'win64'
    filename = os.path.join(LIB_PATH, libdir,'libsvm.dll')
    libsvm = cdll.LoadLibrary(filename)


# Construct constants
SVM_TYPE = ['C_SVC', 'NU_SVC', 'ONE_CLASS', 'EPSILON_SVR', 'NU_SVR' ]
KERNEL_TYPE = ['LINEAR', 'POLY', 'RBF', 'SIGMOID', 'PRECOMPUTED', 'CHI2', 'HI']
...
fillprototype(libsvm.svm_get_kernel_value, c_double, [POINTER(svm_node), POINTER(svm_node), POINTER(svm_parameter)])


def svm_predict(model, x):
    nr_class = model.get_nr_class()
    assert(nr_class == 2)
    dec_values = (c_double * nr_class)()
    node_x = gen_svm_nodearray(x)[0]

    if model.is_probability_model():
        label = libsvm.svm_predict_probability(model, node_x, dec_values)
        if model.get_labels()[0] == -1:
            score = dec_values[1]
        else:
            score = dec_values[0]
    else:
        label = libsvm.svm_predict_values(model, node_x, dec_values)
        score = model.get_labels()[0] * dec_values[0]
    #print dec_values[0], dec_values[1]
    return (label, score)
