...
if sys.platform.startswith('linux'):
    filename = os.path.join(LIB_PATH, 'linux', 'liblinear.so.1')
    liblinear = CDLL(filename)
elif sys.platform.startswith('darwin'):
    filename = os.path.join(LIB_PATH, 'mac', 'liblinear.so.1')
    liblinear = CDLL(filename)
else:
    libdir = 'win32' if platform.architecture()[0] == '32bit' else 'win64'
    filename = os.path.join(LIB_PATH, libdir,'liblinear.dll')
    liblinear = cdll.LoadLibrary(filename)
